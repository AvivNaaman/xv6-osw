...
static int verifyfilecontents(char *path, char *contents) {
  ...
  if ((res = read(fd, buf, contentlen)) != contentlen) {
    printf(stdout, "verifyfilecontents: incorrect length read (%d) for file %s\n",
           res, path);
    close(fd);
    return -1;
  }
  ...
  if ((res = strcmp(contents, buf)) != 0) {
    printf(stdout, "verifyfilecontents: incorrect content read (%s) for file %s\n",
           buf, path);
    return -1;
  }
...
static int umountwithopenfiletest(void) {
  ...
  int res = umount("a");
  if (res != -1) {
    printf(stdout, "umountwithopenfiletest: umount did not fail as expected %d\n",
           res);
    return 1;
  }
...
static int errorondeletedevicetest(void) {
  ...
  if (res != -1) {
    printf(stdout, "errorondeletedevicetest: unlink did not fail as expected %d\n",
           res);
    return 1;
  }
...
static int namespacefiletest(void) {
  ...
  if (pid == 0) {
    ...
    if (open("b/nsfiletest", 0) >= 0) {
      printf(stdout, "namespacefiletest: should not have been able to open file\n");
      return 1;
    }
