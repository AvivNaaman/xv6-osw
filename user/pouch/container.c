...
pouch_status print_clist() {
  ...
  for (i = 0; i < (MAX_TTY - 1); i++) {
    ...
    if ((status = container_name_by_tty(tty, buf)) != SUCCESS_CODE) {
...
pouch_status pouch_container_start(const char* container_name,
  ...
  if ((parent_status = image_exists(image_name)) != SUCCESS_CODE) {
  ...
  if (!daemonize || (daemonize && (pid2 = fork()) == 0)) {
    ...
    if (pid == 0) {  // child
      ...
      if ((child_status = image_name_to_path(image_name, image_path)) !=
          SUCCESS_CODE) {
    ...
    child_error:
      ...
      conf.pid = pid;
      if ((parent_status = write_to_cconf(&conf)) != SUCCESS_CODE) {
...
pouch_status pouch_container_stop(const char* const container_name) {
  ...
  if ((ret = remove_from_pconf(conf.tty_name)) != SUCCESS_CODE) goto error;
...
pouch_status pouch_container_disconnect(const char* const container_name) {
  ...
  pouch_status ret = read_from_cconf(container_name, &conf);
  if (ret != SUCCESS_CODE) {
