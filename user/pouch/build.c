...
static pouch_status parse_import_command(
    struct pouchfile_command* const new_command,
    const struct pouchfile* const pouchfile, const char* const argument) {
  // First, make sure that's the first command in the file.
  if (pouchfile->commands_list_head != NULL) {
    perror("Import command must be the first command in the file");
...
static bool is_command(const char* const line, const char* const command) {
  ...
  return strncmp(current, command, strlen(command)) == 0;
}
...
static pouch_status pouch_build_parse_pouchfile(
  ...
  if (pouchfile_alloc(pouchfile) == ERROR_CODE) {
  ...
  while (extract_line_status == SUCCESS_CODE) {
    ...
    for (const struct command_to_type* cmd_to_type = command_to_type_map;
      ...
      if (pouchfile_add_command(*pouchfile, parser, arg_start) == ERROR_CODE) {
        exit_code = ERROR_CODE;
        goto pouch_commands_error;
      }
  ...
  // Make sure we had a valid import command.
  if (!had_valid_import) {
...
static pouch_status pouch_build_execute(const struct pouchfile* const pouchfile,
  ...
  if ((status = _pouch_container_start(&config)) != SUCCESS_CODE) {
    printf(stderr,
           "Failed to start container with copied image %s, "
           "abort build. status %d\n",
           dest_tag, status);
    return ERROR_CODE;
  }
  printf(stdout, "Stopping container %s\n", config.container_name);
  printf(stderr, "Build completed successfully.\n");
  return SUCCESS_CODE;
}
...
pouch_status pouch_build(const char* const file_name, const char* const tag) {
  ...
  if (status != SUCCESS_CODE) {
    // Cleanup: remove the image if it was created.
    if (pouch_image_exists(tag) == SUCCESS_CODE) {
      if (pouch_image_rm(tag) != SUCCESS_CODE) {
